...
const DataReceive: FC<WebSocketHook> = ({ sendMessage, lastMessage, readyState }) => {
    ...
    if (readyState === ReadyState.OPEN) {
      ...
      intervalRef.current = setInterval(() => {
        try {
          ...
          console.log(`📡 發送數據請求 (${requestIndexRef.current % requests.length + 1}/4):`, 
  ...
  const logReceivedData = (buffer: ArrayBuffer, parsedData: any) => {
    ...
    // 輸出格式化的數據
    console.group(`%c${emoji} ${parsedData.isError ? '錯誤' : '接收'}數據 - ${parsedData.description}`, logStyle);
      ...
      <div className='mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200'>
        <div className='flex items-center justify-between'>
          ...
          <div className='text-xs text-gray-600 space-y-1'>
            ...
            <div>
              成功率: {dataSummary.successRate}% 
              ({dataSummary.success}/{dataSummary.total})
            </div>
          </div>
        </div>
      </div>
            ...
            <div className='space-y-2 max-h-40 overflow-y-auto'>
              {errorData.slice(0, 10).map((error) => (
                <div key={error.id} className='text-sm bg-red-100 p-2 rounded'>
                  ...
                  <span className='text-red-600'>{error.description}: {error.parsedValue}</span>
                </div>
              ))}
            </div>
          </div>
